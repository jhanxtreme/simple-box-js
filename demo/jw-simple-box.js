			...
			try{
				...
				function modal(){
					...
					var settings = {
						...
						ms: 100,
						css: {
							width: config.css ? config.css.width : '50%',
							height: config.css ? config.css.height : '50%',
							borderRadius: config.css ? config.css.borderRadius : '.5em',
							padding: config.css ? config.css.padding : '.5em',
							overflow: config.css ? config.css.overflow : 'auto',
						},
							...
							$("*["+settings.attributes.dataModalTrigger+"=" + sourceName + "]").click(function(e){
								...
								timeout = setTimeout(function(){
									$(settings.classTarget+ "[" +settings.attributes.dataModalSource+"=" + settings.triggerName + "] .content")
										.css({ 'width': settings.css.width, 'height': settings.css.height, 'border-radius': settings.css.borderRadius, 'padding': settings.css.padding, 'overflow': settings.css.overflow })
										.addClass('show');
								}, settings.ms);
							});
						}
				...
				function images(){
					...
					var settings = {
						...
						attributes:{
							dataImageSource: 'data-image-source'
						},
					...
					var initImageInstance = function(elem){
						...
						var dataImageSource = elem.attr(settings.attributes.dataImageSource);
						var classNameSource = (elem[0].getAttribute('class').indexOf(settings.classTarget) >-1);

						//check if the targeted element has attributes 'data-image-source' and 'simple-box'
						if(dataImageSource && classNameSource){
							doPreLoadModal();
							settings.modal.ModalTargetedInstance = new Image();
							settings.modal.ModalTargetedInstance.src = dataImageSource;
							...
							console.log("Data image source not defined.");
						}
					}
